...
Compare wrapping [`stats::rnorm`](https://rdrr.io/r/stats/Normal.html)
for the model with vs without `autotool`:
...
required/optional flags came out of
[`?rnorm`](https://rdrr.io/r/stats/Normal.html) and `formals(rnorm)`.
You wrote one line.
...
description, the title, and every per-argument description.
...
| `"ellmer"` | [`ellmer::tool()`](https://ellmer.tidyverse.org/reference/tool.html) object | the tidyverse `ellmer` LLM client |
...
| `required` | Replace the auto-derived required-arg list (`NULL` = derive; character vector = exact list; [`character()`](https://rdrr.io/r/base/character.html) = nothing required). |
